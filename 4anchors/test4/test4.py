...
if platform == "ios":
    ...
    BluetoothManager = autoclass('BluetoothManager')
    NSString = autoclass('NSString')
    NSURL = autoclass('NSURL')
    NSNotificationCenter = autoclass('NSNotification')
...
if platform == 'ios':
    ...
    class ClientsTable(Screen):
        ...
        def fetch_data(self,point):
            url = f'http://192.168.100.151:5000/Table_Point' #IP of server to connect database
    ...
    if platform == 'ios':
        ...
        class DeviceDispatcher(BluetoothDispatcher):
            def __init__(self, device, rssi, uuid, major, minor):
                ...
                self._rssi = int(device_info.get('rssi', '0'))
                self._uuid = device_info.get('uuid', '')  # Store the UUID
                self._major = device_info.get('major', '') # Store the Major
                self._minor = device_info.get('minor', '') # Store the Minor
            @property
            def title(self) -> str:
        ...
        class MyApp(MDApp,BluetoothDispatcher):
            def build(self):
                ...
                self.target_data_set = [
                    ...
                    {"uuid": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "major": 888},  # A3 HOR
                    {"uuid": "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "major": 111},  # A3 VER
                    {"uuid": "yyyyyyyy-yyyy-yyyy-yyyy-yyyyyyyyyyyy", "major": 888},  # A4 HOR
                    {"uuid": "yyyyyyyy-yyyy-yyyy-yyyy-yyyyyyyyyyyy", "major": 111}   # A4 VER
                ]
                self.found_data = set()  
            ...
            def fetch_data(self, point):
                url = 'http://192.168.100.151:5000/Point' #IP of server to connect database
                try:
            ...
            def fetch_data_map_aoa(self, point):
                url = 'http://192.168.100.151:5000/Point_Map_aoa' #IP of server to connect database
                try:
            ...
            def fetch_data_map_rssi(self, point):
                url = 'http://192.168.100.151:5000/Point_Map_rssi' #IP of server to connect database
                try:
                    response = requests.get(url, json={"point": point})  #send JSON to API
                    print("Response Text:", response.text)  
            ...
            def display_scan_results(self):
                ...
                scanned_info = ''
                A1_V, A1_H, A2_V, A2_H, A3_V, A3_H, A4_V, A4_H = 0,0,0,0,0,0,0,0 #เพิ่ม A3,A4
                ...
                for uuid, major, rssi in self.scan_results:
                    ...
                    if uuid == None:
                       ...
                       ######################## เพิ่ม A3,A4 ###########################
                        elif ((uuid == "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") & (major == 111)):  # ใส่ UUID ของ A3
                            name = "A3(VER)"
                            anchor_id = "A3_V"
                            scanned_info += f"{name}, RSSI: {rssi} dBm\n"
                            A3_V = rssi
                        elif ((uuid == "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") & (major == 888)):  # ใส่ UUID ของ A3
                            name = "A3(HOR)"
                            anchor_id = "A3_H"
                            scanned_info += f"{name}, RSSI: {rssi} dBm\n"
                            A3_H = rssi
                        elif ((uuid == "yyyyyyyy-yyyy-yyyy-yyyy-yyyyyyyyyyyy") & (major == 111)):  # ใส่ UUID ของ A4
                            name = "A4(VER)"
                            anchor_id = "A4_V"
                            scanned_info += f"{name}, RSSI: {rssi} dBm\n"
                            A4_V = rssi
                        elif ((uuid == "yyyyyyyy-yyyy-yyyy-yyyy-yyyyyyyyyyyy") & (major == 888)):  # ใส่ UUID ของ A4
                            name = "A4(HOR)"
                            anchor_id = "A4_H"
                            scanned_info += f"{name}, RSSI: {rssi} dBm\n"
                            A4_H = rssi
            ...
            def send_data(self):
                ...
                url = "http://192.168.100.151:5000/rssi_data" #IP of server to connect database
                ...
                url_datalist = "http://192.168.100.151:5000/rssi_data_list" #IP of server to connect database
                for device in self.scanned_devices:
                    data = {
                ...
                url2 = "http://192.168.100.151:5000/position_aoa" #IP of server to connect database
