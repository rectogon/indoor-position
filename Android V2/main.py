...
if platform == 'android':
    ...
    class MyApp(MDApp,BluetoothDispatcher):
        ...
        def update_found_data_from_manufacturer(self, device_data):
            ...
            self.root.ids.status1.text = " Data sent successfully. Scan stopped."
        ...
        def scan_for_data(self):
            ...
            self.scanning = True
            self.found_data.clear()  # เคลียร์ข้อมูลเก่า
            self.scanner_dispatcher.start_scan()
        ...
        def update_found_data(self, uuid, major, rssi, mac_address=None):
            if rssi != 127:
                ...
                if len(self.found_data) >= 8:
                    Logger.info("ครบ 8 anchor แล้ว หยุด scan ชั่วคราว เพื่อส่งข้อมูล")
                    self.scanner_dispatcher.stop_scan()
                    self.scanning = False
                    
                    # อัพเดตข้อความ UI ตรงนี้เลย
                    self.root.ids.status.text = "หยุดชั่วคราว กำลังส่งข้อมูล..."
                    self.root.ids.status2.text = "หยุดชั่วคราว กำลังส่งข้อมูล..."
                    ...
                    # เริ่ม scan ใหม่ หลังส่งข้อมูล (delay 2 วินาที)
                    Clock.schedule_once(lambda dt: self.scan_for_data(), 2)

                else:
                    self.root.ids.status.text = "Target device found!2"
                    self.root.ids.status2.text = "Target device found!"
...
# ...existing code...
        ...
        def testxy(self, device_data):
            ...
            if self.received_anchor_ids >= self.required_anchor_ids:
                ...
                # หยุด scan ชั่วคราวและอัพเดต UI
                self.scanner_dispatcher.stop_scan()
                
                self.scanning = False
                self.root.ids.status.text = "Paused: Sending data..."
                self.root.ids.status2.text = "Paused: Sending data..."

                # เรียกส่งข้อมูล

                self.process_all_anchor_data()
                    # แสดงข้อความว่าส่งข้อมูลเสร็จแล้ว
                self.root.ids.status.text = "Paused: Sending data..."
                self.root.ids.status2.text = "Paused: Sending data..."
                self.root.ids.status1.text = " Data sent successfully. Scan stopped1."
                self.root.ids.status2.text = " Data sent successfully. Scan stopped."
        ...
        def process_all_anchor_data(self):
            ...
            url_datalist = "http://192.168.100.49:5000/rssi_x_y_list"
            ...
            # รีเซ็ตข้อมูลที่เก็บ anchor ids
            self.reset_tracking_data()

            # Clock.schedule_once(self.show_data_sent_status, 0.1)
            # หยุด scan จริงที่นี่ หลังส่งข้อมูลครบ
            self.stop_scan()
