...
if platform == 'android':
    ...
    REAL_WIDTH_METERS  = 8.0    # 16 cell × 0.5 m
    REAL_HEIGHT_METERS = 12.0   # 24 cell × 0.5 m
    ...
    class MapWidget(Widget):
        def __init__(self, **kwargs):
            ...
            self.start_L_x, self.start_L_y = 131, 345
            self.start_R_x  , self.start_R_y = 907, 345
            self.start_LU_x, self.start_LU_y = 131, 1333
            self.start_RU_x, self.start_RU_y = 776, 990
            #-----
            self.start_A_x, self.start_A_y = 390, 648 
            self.start_B_x, self.start_B_y = 288, 1335 # 4
            self.start_C_x, self.start_C_y = 235, 1335 # 4
            self.start_D_x, self.start_D_y = 597, 1166
            self.start_E_x, self.start_E_y = 597, 1210
            self.start_F_x, self.start_F_y = 234, 1295 # 3
            self.start_G_x, self.start_G_y = 341, 1335 # 4
            self.start_H_x, self.start_H_y = 390, 1335 # 4
            self.start_I_x, self.start_I_y = 287, 1295 # 3
            self.start_J_x, self.start_J_y = 340, 1295 # 3
            self.start_X_x, self.start_X_y = 631, 944 # 3
            # self.start_L2_x, self.start_L2_y = 131, 1378
            # self.start_R2_x, self.start_R2_y = 907, 1378
            
            # Initialize labels but don't add them to the widget yet
            self.coord_label_L = Label(text=f"A3(0, 0)", size_hint=(None, None), color=(1, 0, 0, 1), font_size=24)
            self.coord_label_LU = Label(text=f"A1(11.5, 0)", size_hint=(None, None), color=(1, 0, 0, 1), font_size=24)
            self.coord_label_RU = Label(text=f"A2(11.5, 7.5)", size_hint=(None, None), color=(1, 0, 0, 1), font_size=24)
            self.coord_label_R = Label(text=f"A4(0, 7.5)", size_hint=(None, None), color=(1, 0, 0, 1), font_size=24)
            #-----
            self.coord_label_A = Label(text=f"(2.5, 3.5)", size_hint=(None, None), color=(0, 0, 0, 1), font_size=24)
            self.coord_label_D = Label(text=f"(9.5, 4.5)", size_hint=(None, None), color=(0, 0, 0, 1), font_size=24)
            self.coord_label_E = Label(text=f"(10.5, 4.5)", size_hint=(None, None), color=(0, 0, 0, 1), font_size=24)
            self.coord_label_B = Label(text=f"", size_hint=(None, None), color=(0, 0, 0, 1), font_size=24)
            self.coord_label_C = Label(text=f"", size_hint=(None, None), color=(0, 0, 0, 1), font_size=24)
            self.coord_label_F = Label(text=f"", size_hint=(None, None), color=(0, 0, 0, 1), font_size=24)
            self.coord_label_G = Label(text=f"", size_hint=(None, None), color=(0, 0, 0, 1), font_size=24)
            self.coord_label_H = Label(text=f"", size_hint=(None, None), color=(0, 0, 0, 1), font_size=24)
            self.coord_label_I = Label(text=f"", size_hint=(None, None), color=(0, 0, 0, 1), font_size=24)
            self.coord_label_J = Label(text=f"", size_hint=(None, None), color=(0, 0, 0, 1), font_size=24)
            self.coord_label_X = Label(text=f"", size_hint=(None, None), color=(1, 0, 0, 1), font_size=24)
            ...
            self.coord_label_P = Label(text=f"(0, 0)", size_hint=(None, None), color=(1, 0, 0, 1), font_size=0)        
            ...
            with self.canvas:
                ...
                self.point_A1 = Ellipse(pos=(self.start_LU_x-5, self.start_LU_y-5), size=(15, 15))  
                self.point_A2 = Ellipse(pos=(self.start_RU_x-5, self.start_RU_y-5), size=(15, 15))
                self.point_A3 = Ellipse(pos=(self.start_L_x-5, self.start_L_y-5), size=(15, 15))  
                self.point_A4 = Ellipse(pos=(self.start_R_x-5, self.start_R_y-5), size=(15, 15))
                self.point_A5 = Ellipse(pos=(self.start_X_x-5, self.start_X_y-5), size=(25, 25))
                #-----  
                Color(1, 0, 0, 1)    # Red
                size = 15
                half = size // 2
                ...
                def draw_cross(x, y):
                    # เส้นทแยงมุมจากซ้ายบน -> ขวาล่าง
                    Line(points=[x - half, y + half, x + half, y - half], width=1.5)
                    # เส้นทแยงมุมจากซ้ายล่าง -> ขวาบน
                    Line(points=[x - half, y - half, x + half, y + half], width=1.5)

                # เรียกใช้แทนการวาด Ellipse
                draw_cross(self.start_A_x, self.start_A_y)
                draw_cross(self.start_B_x, self.start_B_y)
                draw_cross(self.start_C_x, self.start_C_y)
                draw_cross(self.start_D_x, self.start_D_y)
                draw_cross(self.start_E_x, self.start_E_y)
                draw_cross(self.start_F_x, self.start_F_y)
                draw_cross(self.start_G_x, self.start_G_y)
                draw_cross(self.start_H_x, self.start_H_y)
                draw_cross(self.start_I_x, self.start_I_y)
                draw_cross(self.start_J_x, self.start_J_y)
                ...
                Color(1, 0, 0, 1)    #red
                self.point_Estimate = Ellipse(pos=(100, 100), size=(0, 0))  

            # ----- วาดเส้นกริด 24 แถว × 16 คอลัมน์ -----
                Color(1, 0, 0, 1)  # สีดำ

                x1, y1 = 131, 345   # ซ้ายล่าง
                x2, y2 = 907, 1333  # ขวาบน
        ...
        def toggle_labels(self):
            if not self.coord_label_L.parent:  # Check if the label is not displayed
                self.coord_label_L.pos = (self.start_L_x - 48, self.start_L_y - 75)
                self.coord_label_R.pos = (self.start_R_x - 48, self.start_R_y - 75)
                self.coord_label_LU.pos = (self.start_LU_x - 48, self.start_LU_y - 75)
                self.coord_label_RU.pos = (self.start_RU_x - 48, self.start_RU_y - 75)
                #-----
                ...
                self.coord_label_G.pos = (self.start_G_x - 48, self.start_G_y - 75)
                self.coord_label_H.pos = (self.start_H_x - 48, self.start_H_y - 75)
                self.coord_label_I.pos = (self.start_I_x - 48, self.start_I_y - 75)
                self.coord_label_J.pos = (self.start_J_x - 48, self.start_J_y - 75)
                self.coord_label_X.pos = (self.start_X_x - 48, self.start_X_y - 75)
                ...
                self.add_widget(self.coord_label_R)
                self.add_widget(self.coord_label_LU)  
                self.add_widget(self.coord_label_RU)
                #----
                ...
                self.add_widget(self.coord_label_G)
                self.add_widget(self.coord_label_H)
                self.add_widget(self.coord_label_I)
                self.add_widget(self.coord_label_J)
                self.add_widget(self.coord_label_X)
                ...
                self.remove_widget(self.coord_label_R)
                self.remove_widget(self.coord_label_LU)  
                self.remove_widget(self.coord_label_RU)
                #----
                ...
                self.remove_widget(self.coord_label_G)
                self.remove_widget(self.coord_label_H)
                self.remove_widget(self.coord_label_I)
                self.remove_widget(self.coord_label_J)
                self.remove_widget(self.coord_label_X)
                # self.remove_widget(self.coord_label_L2)
                # self.remove_widget(self.coord_label_R2)
                self.remove_widget(self.coord_label_P)

        '''def draw_lines_by_angle_L_and_LU(self):
            if not self.angle_drawn_L:
                length_line = 1200
                ...
                with self.canvas:
                    ...
                    for angle in range(start_angle, end_angle+1, 10):  
                        radians_angle = radians(angle)
                        end_LU_x = self.start_LU_x + length_line * cos(radians_angle)  
                        end_LU_y = self.start_LU_y + length_line * sin(radians_angle)
                        line = Line(points=[self.start_LU_x, self.start_LU_y, end_LU_x, end_LU_y], width=1)
                        self.angle_lines_L.append(line)
        ...
        def random_packge(self, n=1):
            self.canvas.after.clear()

            # ใช้ตำแหน่งจริงจาก widget แทนการ hardcode
            rect_x = 131
            rect_y = 345
            rect_w = 776
            rect_h = 990

            #cell_w = rect_w / 16   # = 48.5
            #cell_h = rect_h / 24   # = 41.25
            cell_w = 48.5   # = 48.5
            cell_h = rect_h / 24   # = 41.25
            points = [(rect_x + c*cell_w, rect_y + r*cell_h)
                    for c in range(17) for r in range(25)]

            Logger.info(f"Data is : {points}")
            with self.canvas.after:
                Color(1,0,0,1)
                for x,y in random.sample(points, n):
                    # ถ้าต้องการจุดเป๊ะ ให้วาด cross เล็ก ๆ
                    
                    Ellipse(pos=(x-10, y-10), size=(20, 20))
                    Logger.info(f"X,Y : {x},{y}")
                

        def clear_random_packages(self):
            # ลบวงกลมทั้งหมด
            for dot in self.random_packages:
                self.canvas.remove(dot)
            self.random_packages.clear()

        def real_to_pixel(self, x_m, y_m):
            scale_x = self.image.width / REAL_WIDTH_METERS
            scale_y = self.image.height / REAL_HEIGHT_METERS
            px = self.image.x + x_m * scale_x
            py = self.image.y + y_m * scale_y
            return px, py
    ...
    class MapWidget_Point(Widget):
        def __init__(self, **kwargs):
            ...
            #add map
            self.image = Image(source='image_new2.png', allow_stretch=True, keep_ratio=True)
            self.image.size_hint = (None, None)
            self.image.size = (self.width, self.height)
            self.image.pos_hint = {"center_x": .5, "top": 0.5}  
            self.add_widget(self.image)
        ...
        def on_device(self, device, rssi, advertisement):
            ...
            for item in parsed_data:
                if item.ad_type == 9: #name
                    ...
                    uuid_hex = data_hex[4:36]     # UUID (16 bytes เริ่มที่ offset 2)
                    uuid = uuid_hex

                    major_hex = data_hex[36:40]   # Major (2 bytes)
                    minor_hex = data_hex[40:44]
                    antenna_state = data_hex[47:48]   # Minor (2 bytes)

                    if major_hex and minor_hex and antenna_state:
                        major = int(major_hex, 16)
                        minor = int(minor_hex, 16)
                        antenna_state = int(antenna_state, 16)
                
            Logger.info(f"Discovered UUID:{uuid} Major:{major} Minor:{minor} Anchor:{antenna_state} RSSI:{rssi} ") 
            ...
            if any(filter.get("uuid") == uuid and filter.get("major") == major for filter in self.target_filters):
                ...
                App.get_running_app().update_found_data(uuid, major, rssi, antenna_state)
            # if uuid and major in self.target_filters:
                if major not in self._devices:
                    # Create dispatcher instance for a new device
                    dispatcher = DeviceDispatcher(device, rssi, uuid, major, minor, antenna_state)
                    # Remember address,
                    # to avoid multiple dispatchers creation for this device
                    self._devices[address] = dispatcher
                    Logger.info(f"Scan: device <{name}> address <{address}> added with UUID <{uuid}> and Major <{major}> Anchor <{antenna_state}>")
                    #dispatcher.start()
    ...
    class MyApp(MDApp,BluetoothDispatcher):
        def build(self):
            ...
            self.screen = ScreenManager()  # หรือ MDScreenManager
            map_screen = Screen(name="map")
            root = MapWidget()
            map_screen.add_widget(root)
            self.screen.add_widget(map_screen)
            self.screen = Builder.load_string(KV)

            Clock.schedule_once(lambda dt: root.random_packge(300), 1)
            ...
            # return self.manager
        def random_on_map(self):
            self.root.ids.map_widget.random_packge(n=300)
        ...
        def fetch_data(self, point):
            url = 'http://192.168.100.173:5000/Point' #IP of server to connect database
            try:
