...
//////////////////////////////////////
    ...
    func getCurrentRSSI(uuidString: String) -> Int {
        ...
        // ตรวจสอบว่าเชื่อมต่ออยู่และมี delegate
        if peripheral.state == .connected {
            // ตั้ง delegate ให้แน่ใจ
            peripheral.delegate = self
            
            // อ่านค่า RSSI ใหม่
            peripheral.readRSSI()
            print("📶 Requesting RSSI update for: \(peripheral.name ?? "Unknown")")
        } else {
            print("❌ Device not connected, cannot read RSSI")
        }
        
        // Return ค่าปัจจุบันที่เก็บไว้
        return _rssi
    }
...
func peripheral(_ peripheral: CBPeripheral, didReadRSSI RSSI: NSNumber, error: Error?) {
     guard error == nil else {
        ...
        print("📶 RSSI Updated: \(rssiValue) dBm for device: \(deviceName)")
        ...
        // อัพเดท UI ด้วยข้อมูล RSSI ใหม่
        updateUI(name: deviceName, rssi: rssiValue)
        
        // ส่งข้อมูลไปยัง delegate (ถ้ามี)
        delegate?.didDiscoverDevice(name: deviceName, uuid: deviceUUID, 
                                   rssi: rssiValue, major: _major, minor: _minor)
        
        // อัพเดทข้อมูลใน current device info
