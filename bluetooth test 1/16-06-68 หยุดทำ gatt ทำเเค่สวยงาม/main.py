    ...
    MDBottomNavigation:
        ...
        MDBottomNavigationItem:
            ...
            MDRectangleFlatButton:
                text: "Send"
                text_color: "black"
                on_press: app.connect_gatt(self)
                md_bg_color: app.theme_cls.primary_light
...
class BluetoothGATTApp(MDApp):
    ...
    def build(self):
        ...
        # Initialize BluetoothBridge
        try:
            self.bridge = BluetoothBridge.alloc().init()
            Logger.info("BluetoothGATTApp: BluetoothBridge initialized successfully")
        except Exception as e:
            Logger.error(f"BluetoothGATTApp: Failed to initialize BluetoothBridge: {e}")
            self.update_label(f"❌ Bluetooth initialization failed: {e}")
