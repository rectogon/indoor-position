    ...
    MDBottomNavigation:
        ...
        MDBottomNavigationItem:
            ...
            MDRectangleFlatButton:
                text: "Start_beacon"
                text_color: "black"
                on_press: app.start_beacon(self)
                md_bg_color: app.theme_cls.primary_light
...
class BluetoothGATTApp(MDApp):
    ...
    def build(self):
        ...
        self.load_beacon_regions()
        self.seen_beacons = set()
        ...
        self.target_data_set = [
            ...
            {"uuid": "a1111111111111111111111111111111", "major": 111},
            {"uuid": "a1111111111111111111111111111111", "major": 888},
           
            {"uuid": "b2222222222222222222222222222222", "major": 111},
            {"uuid": "b2222222222222222222222222222222", "major": 888},
    ...
    def start_beacon(self, dt):
        Logger.info("BluetoothBeacon full beacon scan.....")
        self.bridge.startBeaconScan()


    def stop_beacon(self, dt):
        Logger.info("BluetoothBeacon stop beacon scan.....")
        self.bridge.stopBeaconScan()
    ...
    def display_scan_results(self):
        ...
        for uuid, major, rssi in self.scan_results:
            ...
            if uuid == "a1111111111111111111111111111111" and major == 111:
                name = "A1(VER)"
                anchor_id = "A1_V"
                A1_V = rssi

            elif uuid == "a1111111111111111111111111111111" and major == 888:
                name = "A1(HOR)"
                anchor_id = "A1_H"
                A1_H = rssi
            
            elif uuid == "b2222222222222222222222222222222" and major == 111:
                name = "A2(VEA)"
                anchor_id = "A2_V"
                A2_V = rssi

            elif uuid == "b2222222222222222222222222222222" and major == 888:
                name = "A2(VEA)"
                anchor_id = "A2_H"
                A2_H = rssi
            
            elif uuid == "c3333333333333333333333333333333" and major == 111:
                name = "A3(VEA)"
                anchor_id = "A3_V"
                A3_V = rssi

            elif uuid == "c3333333333333333333333333333333" and major == 888:
                name = "A3(VEA)"
                anchor_id = "A3_H"
                A3_H = rssi

            elif uuid == "d4444444444444444444444444444444" and major == 111:
                name = "A4(VEA)"
                anchor_id = "A4_V"
                A4_V = rssi

            elif uuid == "d4444444444444444444444444444444" and major == 888:
                name = "A4(VEA)"
                anchor_id = "A4_H"
                A4_H = rssi
    ...
    def load_beacon_regions(self):
        """โหลดข้อมูล beacon regions จาก Swift"""
        try:
            # ดึงข้อมูล beacon regions ทั้งหมด
            regions_json = self.bridge.getBeaconRegionsAsJSON()
            regions_data = json.loads(regions_json)
            
            Logger.info(f"📍 Loaded {len(regions_data)} beacon regions from Swift")
            
            # แสดงข้อมูล regions
            for region in regions_data:
                uuid = region.get('uuid', '')
                major = region.get('major', 0)
                identifier = region.get('identifier', '')
                
                Logger.info(f"🔹 Region: {identifier} | UUID: {uuid[:8]}... | Major: {major}")
            
            # เก็บข้อมูลไว้ใช้งาน
            self.beacon_regions = regions_data
            self.update_target_data_from_regions()
            
        except Exception as e:
            Logger.error(f"❌ Error loading beacon regions: {e}")

    def update_target_data_from_regions(self):
        """อัพเดท target_data_set จากข้อมูล beacon regions"""
        try:
            if hasattr(self, 'beacon_regions'):
                self.target_data_set = []
                
                for region in self.beacon_regions:
                    uuid = region.get('uuid', '').replace('-', '').lower()
                    major = region.get('major', 0)
                    ...
                    self.target_data_set.append(target_data)
                
                Logger.info(f"✅ Updated target_data_set with {len(self.target_data_set)} regions")
                
        except Exception as e:
            Logger.error(f"❌ Error updating target data: {e}")
            
    def get_beacon_region_info(self):
        """ดึงข้อมูล beacon region แบบละเอียด"""
        try:
            info_json = self.bridge.getBeaconRegionInfoAsJSON()
            info_data = json.loads(info_json)
            
            regions = info_data.get('regions', [])
            uuids = info_data.get('uuids', [])
            count = info_data.get('count', 0)
            
            Logger.info(f"📊 Beacon Region Info:")
            Logger.info(f"   Total Regions: {count}")
            Logger.info(f"   Unique UUIDs: {len(set(uuids))}")
            
            return info_data
            
        except Exception as e:
            Logger.error(f"❌ Error getting beacon region info: {e}")
            return {}
    def get_region_by_identifier(self, identifier):
        """ดึงข้อมูล region ตาม identifier"""
        try:
            region_data = self.bridge.getBeaconRegionByIdentifier(objc_str(identifier))
            
            if region_data:
                uuid = region_data.objectForKey_(objc_str("uuid")).UTF8String()
                major = region_data.objectForKey_(objc_str("major")).intValue()
                identifier = region_data.objectForKey_(objc_str("identifier")).UTF8String()
            ...
            Logger.error(f"❌ Error getting region by identifier: {e}")
            return None
    def display_beacon_regions(self):
        """แสดงข้อมูล beacon regions บน UI"""
        try:
            if hasattr(self, 'beacon_regions'):
                display_text = "📍 Beacon Regions Configuration:\n"
                display_text += "━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                
                for i, region in enumerate(self.beacon_regions, 1):
                    uuid = region.get('uuid', '')
                    major = region.get('major', 0)
                    identifier = region.get('identifier', '')
                    
                    display_text += f"{i}. {identifier}\n"
                    display_text += f"   UUID: {uuid[:8]}...\n"
                    display_text += f"   Major: {major}\n"
                    display_text += "   ─────────────────\n"
                
                display_text += f"Total: {len(self.beacon_regions)} regions"
                
                self.root.ids.label.text = display_text
                
        except Exception as e:
            Logger.error(f"❌ Error displaying beacon regions: {e}")
    def getbe(self):
        getbe = self.bridge.getbe()
        uuid_ = getbe.objectForKey_(objc_str("uuid")).UTF8String()
        self.update_label(f"getbe {uuid_}") 
...
###############################################################
    ...
    # def check_discovered_devicebeacon(self, dt):
    #     try:
    #         device = self.bridge.locationManager()
    #         if not device:
    #             return
    ...
    def get_target_name(self, uuid, major):
        ...
        try:
            if uuid.upper() == "a1111111111111111111111111111111":
                return f"A1 (Major: {major})"
            elif uuid.upper() == "b2222222222222222222222222222222":
                return f"A2 (Major: {major})"
            elif uuid.upper() == "c3333333333333333333333333333333":
                return f"A3 (Major: {major})"
            elif uuid.upper() == "d4444444444444444444444444444444":
                return f"A3 (Major: {major})"
            else:
